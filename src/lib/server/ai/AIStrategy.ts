import type { Die, DiceFace, Player, GameState } from '$lib/types/index.js';
...
export class AIStrategy {
	...
	decideDiceToKeep(dice: Die[], self: Player, state: GameState): number[] {
	...
	selectTarget(
		...
		if (action === 'cutlass') {
			...
			return otherPlayers.sort((a, b) => {
				// Target players with 1-2 lives and no shields first
				...
				// Then target players with most doubloons
